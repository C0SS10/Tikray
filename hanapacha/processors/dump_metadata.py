...
class DumpMetadata:
    ...
    def extract(zip_filename: str):
        ...
        new_format = re.match(r"([A-Za-z]+)_([A-Za-z0-9]+)_(\d{4})-(\d{2})-(\d{2})_(\d{2})-(\d{2})\.zip", zip_filename)
    ...
    def detect_dump_files(prefix: str, date: str, folder: Path):
        ...
        if flexible_found:
            ...
            print(
                f"  ✅ Encontrados {len(flexible_found)} dumps con prefijo '{final_prefix}': {', '.join(flexible_found)}"
            )
